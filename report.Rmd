...
After imputing and standarizing there are still columns with many `NA`s
that the imputing algorithm could not fix. Those columns were eliminated
with the custom function `extractColumnsWithTooManyNAs` below.
The number of predictors went from an initial 159 to 123 after imputing and
finally to 57 after eliminating all `NA`s remaining.
